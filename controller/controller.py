...
class OpenFlowController(object):
    ...
    def server_loop(self, ofp_tcp_listen_port, ofp_ssl_listen_port):
        ...
        LOG.info('Stream Server loop, port is %s' % ofp_tcp_listen_port)
...
class Datapath(ofproto_protocol.ProtocolDesc):
    ...
    def _recv_loop(self):
        ...
        while self.state != DEAD_DISPATCHER:
            ...
            while buf_len >= min_read_len:
                ...
                if buf_len < msg_len: # 有可能开始读的只是报文头，但是msg_len是真实的报文长度，这个时候需要退出并继续读
    ...
    def send_msg(self, msg, close_socket=False):
        ...
        # if msg.cls_msg_type == 0:
        #     print "msg.buf len is %s" % len(msg.buf)
    ...
    def serve(self):
        ...
        # print "self.ofproto_parser is %s" % self.ofproto_parser
    ...
    def send_packet_out(self, buffer_id=0xffffffff, in_port=None,
        ...
        # print "packet_out is %s" % packet_out
...
def datapath_connection_factory(socket, address):
    # LOG.info('remote socket is:%s, address is %s', socket.getpeername(), address)
